...
class FixHeaders2():
    ...
    def correct_header_value(self, key, new_value):
        print('Do you want to save the changes to the existing file? [y/n]')
        answer = input()
        if answer == 'y':
            with fits.open(self._path, mode='update') as self.hdul:
                self.hdul[0].header[key] = new_value
                self.hdul.flush()
        elif answer == 'n':
            self.hdul = fits.open(self._path)
            self.hdul[0].header[key] = new_value
